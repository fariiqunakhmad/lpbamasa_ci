 ...
 */
class Biaya_kuliah1 extends CI_Controller {
    ...
    function load_form() {
        if ($this->uri->segment(3)!=NULL && $this->uri->segment(4)!=NULL){
            ...
            $data['record']     = $this->biaya_kuliah_m->get($param);
        ...
        $view['topnav']     = 'admin_topnav';
        $view['sidenav']    = 'admin_sidenav';
        $view['content']    = 'form_biaya_kuliah';
        $this->page->view($view, $data);
    }
    function insert() {
        $data=array(
        ...
        $this->biaya_kuliah_m->insert($data);
    ...
    function update() {
        ...
        $this->biaya_kuliah_m->update($id, $data);
        redirect($this->uri->segment(1), 'refresh');
    }
    function delete() {
        $id=array($this->uri->segment(3), $this->uri->segment(4));
        $this->biaya_kuliah_m->delete($id);
        redirect($this->uri->segment(1), 'refresh');
